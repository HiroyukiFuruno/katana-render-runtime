...
FORMAL_BOT = "chatgpt-codex-connector"
BODY = "Closes #64"
...
def successful_state() -> tuple[
    ...
    pull_request = {
        ...
        "reviews": [
            ...
            formal_review(HEAD, "2026-08-29T03:02:30Z"),
        ],
    }
    threads = [
        {
...
class VerifyPrReadyTest(unittest.TestCase):
    ...
    def test_accepts_actual_shaped_codex_formal_reviews(self) -> None:
        ...
        pull_request["reviews"] = [
            formal_review(HEAD, "2026-08-29T03:01:30Z", body=actual_shape),
            formal_review(HEAD, "2026-08-29T03:02:30Z", body=actual_shape),
        ]
        self.assertEqual(self.errors(pull_request=pull_request, threads=threads, comments=comments), [])

    def test_rejects_formal_review_with_noncanonical_identity(self) -> None:
        for field, value in (
            ("login", "chatgpt-codex-connector[bot]"),
            ("databaseId", 199175423),
            ("id", "BOT_kgDOC98s_h"),
            ("__typename", "User"),
        ):
            with self.subTest(field=field):
                ...
                for review in reviews:
                    ...
                    author[field] = value
                errors = " ".join(self.errors(pull_request=pull_request))
                self.assertIn("review bot 完了記録", errors)

    def test_rejects_formal_review_with_malformed_body_or_commit(self) -> None:
        for mutate in (
            lambda review: review.__setitem__("body", "### 💡 Codex Review\n"),
            lambda review: review["commit"].__setitem__("oid", HEAD.upper()),
            lambda review: review.__setitem__(
                "body", review["body"] + "\n<details>nested</details>"
            ),
            lambda review: review.__setitem__("submittedAt", "2026-08-29 03:01:30"),
        ):
            with self.subTest(mutate=mutate):
                ...
                for review in reviews:
                    mutate(review)
                self.assertIn("review bot 完了記録", " ".join(self.errors(pull_request=pull_request)))

    def test_rejects_duplicate_formal_and_issue_comment_evidence_in_one_phase(self) -> None:
        pull_request, _, comments = successful_state()
        comments.insert(1, no_issues_review(3, HEAD, created_at="2026-08-29T03:01:45Z"))
        self.assertIn("initial-final間の Codex review evidence が曖昧です", self.errors(pull_request=pull_request, comments=comments))
    ...
    def test_rejects_no_issues_comment_with_nonimmutable_bot_identity(self) -> None:
        for field, value in (
            ("login", FORMAL_BOT),
            ("id", 1),
            ("node_id", "BOT_lookalike"),
            ("type", "User"),
        ):
            with self.subTest(field=field):
                comment = no_issues_review(3, HEAD)
                user = comment["user"]
                assert isinstance(user, dict)
                user[field] = value
                self.assertEqual(
                    subject._no_issues_comment_completion_times(
                        [comment], BOT, HEAD, "2026-08-29T03:01:00Z"
                    ),
                    [],
                )
        ...
        for case in cases:
            with self.subTest(case=case.get("name")):
                ...
                if accepted:
                    ...
                    self.assertIsInstance(user.get("login"), str)
    ...
    def test_open_pull_requests_fails_closed_when_later_page_changes_at_race_fence(self) -> None:
        def payload(number: int, *, next_page: bool) -> dict[str, object]:
            return {
                "data": {"repository": {
                    "defaultBranchRef": {"name": "master"},
                    "pullRequests": {
                        ...
                        "pageInfo": {
                            ...
                            "endCursor": "page-2" if next_page else None,
                        },
                    },
                }},
            }

        first = payload(71, next_page=True)
        second = payload(72, next_page=False)
        changed_second = payload(73, next_page=False)
        with patch.object(
            subject,
            "_gh_json",
            side_effect=[first, second, first, changed_second],
        ):
            with self.assertRaisesRegex(ValueError, "open pull requests changed"):
                subject._open_pull_requests("owner/repo")
    ...
    def test_main_rechecks_app_bound_producer_before_and_after_readiness(self) -> None:
        ...
        # Initial readiness and both complete final fences must validate the
        # App-bound producer; a post-review identity-only check is insufficient.
        self.assertEqual(check_run_calls, 3)
    ...
    def test_review_reader_selects_identity_body_and_refuses_a_raced_projection(self) -> None:
        ...
        changed["data"]["repository"]["pullRequest"]["reviews"]["nodes"][0]["body"] += "\nchanged"
        ...
        with patch.object(subject, "_gh_json", side_effect=gh_json):
            with self.assertRaisesRegex(ValueError, "reviews changed during pagination"):
                subject._reviews("owner/repo", 72)
        query = next(argument for argument in calls[0] if "reviews(first: 100" in argument)
        self.assertIn("... on Bot { databaseId id }", query)
        ...
        for connection_name, reader in (
            ...
            with self.subTest(connection=connection_name):
                ...
                def gh_json(*arguments: str) -> object:
                    ...
                    cursor = next(
                        (
                            argument.removeprefix("cursor=cursor-")
                            for argument in arguments
                            if argument.startswith("cursor=cursor-")
                        ),
                        None,
                    )
                ...
                self.assertEqual(
                    len(calls),
                    (
                        subject._MAX_REVIEW_CONNECTION_PAGES * 2
                        if connection_name == "reviewThreads"
                        else subject._MAX_REVIEW_CONNECTION_PAGES * 2
                    ),
                )
    ...
    def test_reviews_fails_closed_when_later_page_changes_at_race_fence(self) -> None:
        self.use_complete_review_reader()
        first = rate_limited({
            "data": {"repository": {"pullRequest": {"reviews": {
                "nodes": [{"id": "review-1"}],
        ...
        changed_second["data"]["repository"]["pullRequest"]["reviews"]["nodes"] = [  # type: ignore[index]
            {"id": "review-3"}
        ]
        with patch.object(
            subject,
    ...
    def test_review_threads_fails_closed_when_later_page_changes_at_race_fence(self) -> None:
        first_page = rate_limited({
            "data": {"repository": {"pullRequest": {"reviewThreads": {
                "nodes": [],
                "pageInfo": {"hasNextPage": True, "endCursor": "page-2"},
            }}}}
        })
        second_page = rate_limited({
            "data": {"repository": {"pullRequest": {"reviewThreads": {
                "nodes": [{
                    "id": "thread-page-2",
                    "isResolved": True,
        ...
        changed_second_page = deepcopy(second_page)
        changed_second_page["data"]["repository"]["pullRequest"]["reviewThreads"]["nodes"][0]["isResolved"] = False  # type: ignore[index]
        with patch.object(
            subject,
            "_gh_json",
            side_effect=[first_page, second_page, first_page, changed_second_page],
        ):
            with self.assertRaisesRegex(ValueError, "reviewThreads changed"):
                subject._review_threads("owner/repo", 72)
    ...
    def test_rest_pages_fail_closed_when_later_page_changes_at_race_fence(self) -> None:
        first = [{"id": number} for number in range(100)]
        second = [{"id": 100}]
        changed_second = [{"id": 101}]
        with patch.object(
            subject,
            "_gh_json",
            side_effect=[first, second, first, changed_second],
    ...
    def test_review_graphql_page_cap_fits_the_125_head_shared_ledger(self) -> None:
        ...
        review_snapshot_requests = (
            2  # initial and final snapshots
            * 2  # reviews and reviewThreads
            * 2  # initial page read and race-fence reread
            * subject._MAX_REVIEW_CONNECTION_PAGES
        )
        bounded_review_requests = (
            subject._MAX_ALL_WRITER_TARGETS * review_snapshot_requests
        )
        self.assertEqual(subject._MAX_ALL_WRITER_TARGETS, 125)
        ...
        # Nested thread-comment overflow is deliberately not admitted by this
        # capacity proof: the shared ledger rejects it before the reserved
        # floor can be spent.
        nested_overflow_requests = subject._MAX_ALL_WRITER_TARGETS * (
            review_snapshot_requests
            + 2 * subject._MAX_REVIEW_THREAD_COMMENT_FOLLOW_UP_PAGES
        )
        self.assertGreater(
            nested_overflow_requests,
            subject._GRAPHQL_REVIEW_BUDGET - subject._GRAPHQL_REMAINING_FLOOR,
        )
    ...
    def test_reads_all_review_thread_comment_pages_within_follow_up_limit(self) -> None:
        ...
        def gh_json(*arguments: str) -> object:
            calls.append(arguments)
            cursor = next(
                argument.removeprefix("commentsCursor=")
                for argument in arguments
                if argument.startswith("commentsCursor=")
            )
            page_number = (
                1
                if cursor == "initial-cursor"
                else int(cursor.removeprefix("cursor-")) + 1
            )
            return rate_limited({
                "data": {
                    "node": {
        ...
        self.assertEqual(
            len(calls), subject._MAX_REVIEW_THREAD_COMMENT_FOLLOW_UP_PAGES * 2
        )
...
class StrictGovernanceCheckRunTest(unittest.TestCase):
    ...
    def _allow_ready(
        ...
        *,
        ...
        post_review_readiness_error: Exception | None = None,
        ...
        # The post-review governance fence reads the same immutable source a
        # second time.  Keep the final supplied source stable unless a test
        # deliberately supplies a changed successor.
        source_queue = [*sources, deepcopy(sources[-1])]
        ...
        readiness_fence_calls = 0

        def observed_readiness_fence(**_kwargs: object) -> None:
            nonlocal readiness_fence_calls
            readiness_fence_calls += 1
            if trace is not None:
        ...
        with patch.object(subject, "_gh_json", side_effect=gh_json), patch.object(
            ...
            subject,
            "_review_threads",
            side_effect=lambda *_args, **_keywords: next_snapshot(
    ...
    def test_allow_ready_refreshes_review_evidence_after_final_governance_fence(self) -> None:
        ...
        self.assertEqual(len(governance_indexes), 3)
    ...
    def test_allow_ready_rejects_a_post_review_full_fence_change(self) -> None:
