...
# 境界。初回と最終の review snapshot は各 page を race fence で再読する。
# 2 page に制限して、共有 all-writer ledger に完全な fail-closed snapshot
# の予算を残す。
_MAX_REVIEW_CONNECTION_PAGES = 2
# all-open writer は terminal segment で最大125 headを処理する。
# review/reviewThreads と race-fence の再読は共有 installation ledger に課金されるため、
# この上限は writer と整合させる。
_MAX_ALL_WRITER_TARGETS = 125
...
def _is_exact_codex_review_bot_rest(value: object, review_bot: str) -> bool:
    """同名アカウントの偽装を防ぐためRESTの不変なBot識別情報を照合する。"""

    if review_bot != _CODEX_REVIEW_BOT_LOGIN or not isinstance(value, Mapping):
        return False
    return (
        value.get("login") == _CODEX_REVIEW_BOT_LOGIN
        and type(value.get("id")) is int
        and value.get("id") == _CODEX_REVIEW_BOT_DATABASE_ID
        and value.get("node_id") == _CODEX_REVIEW_BOT_NODE_ID
        and value.get("type") == _CODEX_REVIEW_BOT_TYPE
    )
...
def _no_issues_comment_completion_times(
    ...
    for comment in comments:
        if not _is_exact_codex_review_bot_rest(comment.get("user"), review_bot):
            continue
...
def _open_pull_requests(repository: str) -> list[dict[str, object]]:
    ...
    page_count = 0
    page_snapshots: list[tuple[list[str], object]] = []
    while True:
        arguments = [
            "api",
            "graphql",
        ...
        page_snapshots.append((list(arguments), payload))
        if not isinstance(payload, Mapping):
        ...
        if has_next_page is False:
            for page_arguments, page_payload in page_snapshots:
                reread = _gh_json(*page_arguments)
                if _stable_graphql_payload(reread) != _stable_graphql_payload(page_payload):
...
def _rest_pages(endpoint: str, *, item_key: str | None = None) -> list[object]:
    ...
    for page_number in range(1, _MAX_REST_PAGES + 1):
        ...
        page_snapshots.append(([
            "api", _rest_page_endpoint(endpoint, page_number)
        ], payload))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not arguments.require_draft:
        post_review_required_checks = _required_status_check_snapshot(
            repository, base_branch
        )
        if post_review_required_checks != initial_required_checks:
            raise ValueError("required status checks changed during readiness check")
        post_review_governance_evidence: dict[str, object] = {}
        post_review_governance_error = _governance_check_error(
            repository,
            arguments.pr,
            base_branch,
            base,
            head,
            initial_body_sha256,
            post_review_governance_evidence,
            exclude_trusted_governance_check=arguments.exclude_trusted_governance_check,
            required_checks=post_review_required_checks,
        )
        if (
            post_review_governance_error is not None
