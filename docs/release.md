...
PR 作成前には `lefthook run pre-pr` を実行し、対象 version 以前の完了済み OpenSpec change が `openspec/changes/archive/` に移動済みであることを確認する。
...
`just release-check` は、release 予定 version 以前の OpenSpec change が active 側に残っている場合に失敗する。
内容は次の通り。
...
4. `katana-diagram-renderer` を crates.io に公開
5. crates.io で `katana-diagram-renderer` が見えるまで待機
6. `katana-diagram-renderer-cli` を crates.io に公開
