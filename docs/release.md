...
`katana-render-runtime` は `katana-diagram-renderer` の履歴を引き継ぎ、`v0.3.0` から新 crate として公開する。
...
- `katana-diagram-renderer` wrapper の梱包（package）収録対象確認
- `katana-diagram-renderer-cli` の梱包（package）収録対象確認

`katana-diagram-renderer` wrapper と `katana-diagram-renderer-cli` は `katana-render-runtime` を先に公開しないと crates.io 上で依存解決できない。
そのため取り込み依頼（Pull Request）時点では `katana-render-runtime` を事前実行（dry-run）し、wrapper と CLI は収録対象確認までに留める。
公開順序は `katana-render-runtime`、wrapper、CLI の順に固定する。
...
7. `katana-diagram-renderer` wrapper を crates.io に公開
8. crates.io で `katana-diagram-renderer` が見えるまで待機
9. `katana-diagram-renderer-cli` を crates.io に公開
