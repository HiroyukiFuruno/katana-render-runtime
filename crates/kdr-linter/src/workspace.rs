...
impl WorkspaceModel {
    pub fn load(root: &Path) -> Result<Self, KdrLintError> {
    ...
    fn collect_rs_files(root: &Path) -> Result<Vec<SourceFile>, KdrLintError> {
    ...
    fn parse_file(path: &Path) -> Result<SourceFile, KdrLintError> {
