...
impl ManifestBoundaryRule {
    pub(super) fn check(root: &Path) -> Result<Vec<Violation>, KcfLintError> {
    ...
    fn check_cli_manifest(
        ...
        if dependencies.iter().any(|it| it == "katana-canvas-forge") {
            return Ok(());
        }
    ...
    fn is_renderer_boundary_violation(dependency: &str) -> bool {
        UiDependencyPolicy::is_ui_dependency(dependency) || dependency == "katana-canvas-forge-cli"
    }
    ...
    fn manifest_violation(path: &Path, dependency: String) -> Violation {
        Violation::new(
            ...
            1,
            "renderer-boundary",
            format!("renderer/export crate must not depend on UI or CLI crate `{dependency}`."),
        )
    }
}
...
impl ManifestReader {
    fn read(path: &Path) -> Result<toml::Value, KcfLintError> {
        let source = std::fs::read_to_string(path).map_err(|source| KcfLintError::Read {
            path: path.to_path_buf(),
            source,
        })?;
        toml::from_str(&source).map_err(|source| KcfLintError::TomlParse {
            path: path.to_path_buf(),
            source,
        })
    }
