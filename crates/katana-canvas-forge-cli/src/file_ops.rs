...
mod tests {
    ...
    fn writes_and_reads_text() -> Result<(), Box<dyn std::error::Error>> {
        let path = std::env::temp_dir().join(format!("kcf-file-{}.txt", std::process::id()));
    ...
    fn read_and_write_errors_keep_path_context() {
        let missing =
            std::env::temp_dir().join(format!("kcf-file-missing-{}.txt", std::process::id()));
        let unwritable = std::env::temp_dir()
            .join(format!("kcf-file-missing-dir-{}", std::process::id()))
            .join("out.txt");
