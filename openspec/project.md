# katana-diagram-renderer OpenSpec

## Project

`katana-diagram-renderer`（KDR）は、Mermaid / Draw.io / PlantUML / mathなどの外部描画を担うversioned rendering library。KatanA はこれを git dependency としてconsumeするだけ。描画実装・版管理・採点運用はここで行う。

HTML / PDF / PNG / JPEG などの文書出力は KDR の責務外とし、KDR は外部図形描画と reference score に集中する。

## Design Principles

- KatanA の public interface に kdr 固有の型を漏らさない。KatanA が見るのは `Renderer` trait と中立 DTO のみ。
- v0.1.0 transfer では KatanA 側で定義済みの描画 interface を正本として完全踏襲する。
- 文書出力責務はKDRに追加しない。Markdown viewer/exportはKDR外で扱う。
- `egui` / KatanA UI state に依存しない。将来 KatanA が egui から脱却しても kdr は無影響。
...
- `v0.1.0`: KatanA 既存 rendering runtime の忠実移植（transfer）。
  ...
  - 開発用 kdr CLI（`render` / `reference-update` / `compare` / `bench`）
...
- `v0.1.1`: Mermaid.js / Draw.io.js runtime asset version pinning
  - Mermaid.js と Draw.io.js の取り込み version を kdr で固定する
...
- `v0.1.6`: KDV export handoff
  - 不要になった export CSS debug 計画はKDR release番号から外す
  - 旧 export/debug 論点はKDV側へ移譲する
  - KDR側は Mermaid / Draw.io rendering、runtime asset、reference score に責務を絞る
- `v0.2.0`: PlantUML SVG renderer と multi platform runtime 対応
  - KDR の公開 API と CLI で PlantUML を SVG または raw code block として返す
  - `plantuml.jar` は固定 URL / checksum で初回に保存領域（cache）へ取得し、crates.io package には JAR 本体を含めない
  - 保存領域（cache）は OS 既定値、`KDR_PLANTUML_CACHE_DIR`、CLI `--cache-dir`、API `plantuml_cache_dir` で上書きできる
  - `libjvm` を macOS / Linux / Windows で解決し、解決できない場合は警告付き raw fallback で正常継続する
  - PlantUML 公式 theme / dark mode を使い、CLI help と公開 API で指定可能 theme 名を確認できるようにする
  - 公式 9 種類 fixture の比較で minimum score 100 を release gate にする
- `v0.3.0`: KDV移譲計画へ変更
  - PDF viewer renderingはKDV側へ移す
  - KDR側では外部描画referenceとscoreを維持する
- `v0.4.0`: KDV移譲計画へ変更
  - Office viewer renderingはKDV側へ移す
  - KDR側ではOffice viewer責務を持たない
- `v0.4.x`: バグ取りと score 向上
  - Mermaid / Draw.io / 外部描画reference のバグ修正
...
> **方針**: KDRの責務は外部描画、runtime asset管理、reference scoreである。文書出力や viewer はKDR外で扱う。
...
### katana-diagram-renderer の移行

kdr は最初から egui 非依存（neutral only）のため、UI フレームワーク移行の影響を受けない。
