...
原因は ZenUML が Mermaid 本体だけでは描画されず、ZenUML 対応JSを取り込み、Mermaid runtime に外部 diagram として登録していないことにある。v0.1.2 では `28-zen-uml.md` を未対応扱いにせず、コードブロックの構文木（AST）から Mermaid / ZenUML を判定し、取り込んだ ZenUML 対応JSで実際に描画する。
