...
KatanA は `DiagramThemeSnapshot` を作って kcf の `RenderInput` に渡している。

- `crates/katana-core/src/markdown/diagram_backend/katana_backend.rs`
  - `kcf_input()` が `RenderPolicy { background, cache_profile, .. }` と `RenderContext { theme_fingerprint, document_id }` を設定
...
しかし kcf 側の renderer は `RenderInput.source` だけを使い、theme 関連の field を読んでいない。

- `crates/katana-canvas-forge/src/renderer/backends.rs:34-40`
...
- `crates/katana-canvas-forge/src/renderer/backends.rs:67-74` も同様
- `crates/katana-canvas-forge/src/markdown/mermaid_renderer/render.rs:33`
  - `let preset = DiagramColorPreset::current();` で process global を読む
- `crates/katana-canvas-forge/src/markdown/color_preset/types.rs:24`
  - `DARK_MODE: AtomicBool = AtomicBool::new(true)` で初期値が dark
- `crates/katana-canvas-forge/src/renderer/fingerprint.rs:24`
...
- `RenderInput` の theme 情報を、Mermaid / Draw.io の実描画に必ず使う
- cache fingerprint を `RenderInput` 由来の実効 theme で計算する
- `DiagramColorPreset::current()` の global state が dark のままでも、`RenderInput` で light が指定されれば light で描画される
- 既存 consumer (KatanA) との後方互換は、`RenderContext` に新 field を任意（Option）で追加する形で保つ

## Non-Goals

- KatanA `DiagramThemeSnapshot` の構造を kcf 側仕様に合わせて変える（KatanA v0.22.13 で扱う）
- `DiagramColorPreset` の色 palette を変更する
...
```rust
// crates/katana-canvas-forge/src/renderer/api.rs
...
`color_preset` module に `effective_preset(input: &RenderInput) -> DiagramColorPreset` を追加する。
...
`RenderContext.theme` が `Some` の場合は、global を一切参照しない。両者の優先順位を doc comment で明示する。

### 6. 回帰テスト

`crates/katana-canvas-forge/src/renderer/backends.rs` または隣接 test module に追加:
...
- `light_input_renders_with_light_preset_even_when_global_is_dark`
  ...
  - 描画層へ渡された preset が light 値であることを fake runtime / spy で確認
- `light_and_dark_inputs_produce_distinct_outputs`
  - 同一 source で light / dark の `RenderInput` を render
  - SVG 出力または描画 request が異なる
...
- `RenderInput.context.theme` が反映されたことを示す renderer 単体テストが追加されている
- `DARK_MODE` を true に固定しても、light の `RenderInput` が light で描画される
