...
- reference snapshot 更新は全量評価（full evaluation）用 artifact と代表ケース評価（representative evaluation）用 artifact の両方を更新する
...
- 取り込み後に full / representative の reference snapshot を再生成する
- 取り込み後に local full compare と CI/CD representative compare を実行し、score 低下を検知する

CI/CD の通常経路は reference snapshot を再生成しない。CI/CD は representative compare だけを実行し、version 更新 PR の中で git 管理済み artifact 差分と baseline 差分を review する。
