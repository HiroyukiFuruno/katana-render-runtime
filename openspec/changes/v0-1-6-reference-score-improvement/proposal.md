...
この範囲は旧 v0.1.1 / v0.1.2 として Jules 側へ渡していたが、粒度が大きく進捗が停滞している。Mermaid ZenUML / unsupported fixture handling は v0.1.2 で閉じ、score 改善（score improvement）を v0.1.6 の最終フェーズへ送る。不要になった export CSS debug 計画はKCF release番号から外し、旧 export/debug 論点は v0.1.6 のKDV移譲記録で扱う。

v0.1.6 の対象は CI 代表ケースだけではなく、full compare が見る全 supported pattern である。完了基準は Draw.io / Mermaid ともに 99 点以上とし、CI compare は高速な代表確認、full compare は完了判定として扱う。
...
- `openspec/changes/v0-1-6-reference-score-improvement/` — 本 change の仕様とタスク
