...
- `vendor/drawio/<version>/drawio.min.js` + `.sha256` で Draw.io JS を版固定管理する
  - v0.1.0 の Mermaid.js 版固定（`vendor/mermaid/<version>/`）と同じ方式を踏襲する
  - `just VERSION=<version> drawio-js-update` で版更新・checksum・スナップショット再生成を一括実施する
  - `DRAWIO_JS_VERSION` 定数を crate から公開し、テスト側は定数を参照してバージョンを明示する
  - バージョンを変えずに寸法スナップショットが変わった場合は CI で即検知する
