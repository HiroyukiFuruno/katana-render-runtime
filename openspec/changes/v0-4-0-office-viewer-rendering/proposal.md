...
v0.4.0 では Word / Excel / PPTX に限定して、編集や Office 書き出しではなく、既存 viewer e2e と同じ考え方で表示結果を検証できる rendering surface を追加する。
...
- viewer e2e、reference snapshot、差分比較、lint、AST lint、release gate を品質確認に含める
...
- `test/e2e/viewer/` — Office rendering の viewer e2e case と reference snapshot
- `tests/fixtures/office/` — Word / Excel / PPTX の最小 case と代表 case
- `Justfile` — Office reference 更新、viewer e2e、品質 gate recipe
