...
v0.1.0 はまだ KatanA に取り込まないため release する。ただし、score が低いまま KatanA 側の検証を削ぐと品質低下を見逃すため、v0.1.4 で score 改善を完了してから KatanA 側への取り込み（consume）を判断する。

この範囲は旧 v0.1.1 / v0.1.2 として Jules 側へ渡していたが、粒度が大きく進捗が停滞している。v0.1.x では先に小さい後続フェーズを閉じ、score 改善（score improvement）は v0.1.4 の最終フェーズへ送る。
...
v0.1.4 は「最低点を 99 以上へ上げる最終フェーズ」である。
...
unsupported fixture は暗黙 skip しない。Mermaid ZenUML / unsupported fixture handling は v0.1.2 で固定し、v0.1.4 はその metadata を前提に supported pattern の score 改善へ集中する。
...
修正しない既知差分は、理由と現在 score を report に残す。ただし、その case が supported fixture である限り、99 未満のまま v0.1.4 完了にはしない。どうしても v0.1.4 で直せない場合は、ユーザー確認を受けて別 change へ明示的に送る。
...
2. 失敗 fixture を含む最小 fixture directory だけを `just drawio-compare <fixture-dir> 99 tmp/kcf-v0.1.4-reference-score-improvement/<case>` で再実行する
3. `tmp/kcf-v0.1.4-reference-score-improvement/<case>/comparison` の official PNG、kcf PNG、diff / report を見て差分を一種類だけ分類する
4. 分類に対応する最小ファイルだけを修正する
5. 同じ case を再実行し、score が上がったことを確認する
6. score が 99 以上になったら次の case に進む
7. 失敗が full compare でだけ出る場合は、該当 fixture を含む最小 fixture directory だけを `just drawio-compare <fixture-dir> 99 tmp/kcf-v0.1.4-reference-score-improvement/full-<slug>` で切り出す
...
- `just mermaid-compare-full 99`
- `just check`
- `just VERSION=v0.1.4 release-check`
- `npx -y @fission-ai/openspec validate "v0-1-4-reference-score-improvement" --strict`

Mermaid ZenUML 由来の failure も supported fixture の score failure として扱い、v0.1.4 の score 改善完了条件に含める。
