...
一方で、移植時の full compare では Draw.io official diagrams の `i18n` と `link` が 99 点に届かず、代表ケースにも既知下限が残っている。これは v0.1.0 の公開を止めるのではなく、v0.1.2 に繰り上げる小規模 patch として扱う。

この範囲は旧 v0.1.1 として Jules 側へ渡していたが進捗が停滞している。現 v0.1.2 の export CSS debug は他repositoryの開発待ちがあるため v0.1.3 へ送り、score 改善（score improvement）を v0.1.2 に繰り上げる。

v0.1.2 の対象は CI 代表ケースだけではなく、full compare が見る全 supported pattern である。完了基準は Draw.io / Mermaid ともに 99 点以上とし、CI compare は高速な代表確認、full compare は完了判定として扱う。
...
- unsupported fixture は暗黙 skip せず、v0.1.2 で扱わない理由と後続 change を明示する
- score report と contact sheet を PR review で確認できる成果物として残す

## Non-Goals

- v0.1.0 release を延期しない
- Mermaid ZenUML fixture の対応は v0.1.4 に送る
...
- `openspec/changes/v0-1-2-score-improvement/` — 本 change の仕様とタスク
