...
現在の KDR は、Mermaid と Draw.io の browser 互換 shim、DOM helper、SVG postprocess、render entrypoint を多数の `.js` ファイルとして保持し、Rust 側の `include_str!` で読み込んだ順に V8 へ渡している。
...
```text
crates/katana-diagram-renderer/src/markdown/diagram_runtime/
  source/
    shared/
    mermaid/
    drawio/
...
現状の `kdr-linter` は Rust file を対象に、関数長、禁止 method、禁止 type、lazy macro、`allow` 属性、CLI / library 境界を検査している。一方で、TypeScript source、generated bundle、Rust 側 `include_str!` の参照先、bundle checksum の同期は検査対象外である。
...
TypeScript の詳細 AST 解析は Biome / TypeScript compiler に任せ、`kdr-linter` は repo 固有の構造契約を担当する。
...
- [Risk] AST lint が TypeScript parser まで抱えて複雑化する → TypeScript 文法の詳細は Biome / TypeScript compiler に寄せ、kdr-linter は repo 階層と Rust include 境界に限定する
