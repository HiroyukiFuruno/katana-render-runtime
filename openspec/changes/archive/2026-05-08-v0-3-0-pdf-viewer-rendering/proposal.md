...
責務再整理により、PDF viewer renderingはKDVへ移譲する。KDRではこのchangeを実装開始しない。
...
- KDR側のv0.3.0としてはPDF viewer renderingを開始しない
- KDV側OpenSpecへPDF viewerのpage artifact、metadata、diagnostics、windowingを移す
- KDR側には外部描画referenceやscoreの保守だけを残す
...
- `openspec/changes/v0-3-0-pdf-viewer-rendering/` — KDR側では移譲記録として維持
