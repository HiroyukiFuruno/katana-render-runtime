# Tasks: katana-diagram-renderer v0.1.0 transfer

## 実施状況メモ（2026-05-05）

- [x] kdr 側への Mermaid / Draw.io / export / score 移植は release branch 上で実装済み
...
- [x] KatanA 由来の Mermaid / Draw.io / export 検証観点は kdr 側 integration test と reference compare に移管済み
...
- [/] Justfile の `kdr-*` recipe 名は v0.1.0 公開前品質として整理済み
...
- [/] KatanA 側で kdr v0.1.0 tag を consume する確認は KatanA 側の関心に分離し、kdr v0.1.0 の完了条件から外す
...
KatanA 側の移植元を漏れなく洗い出し、kdr 側の crate / module / CLI / vendor 配置へ写像する。

### 書き込み範囲

- `openspec/`
- `crates/katana-diagram-renderer/src/`
- `crates/katana-diagram-renderer-cli/src/`
- `vendor/`
- `tests/`
...
- [x] 1.7 KatanA UI state、preview state、workspace state を kdr に持ち込まない境界を確認する
...
## 2. kdr 公開 API で KatanA 既存能力を維持する

### 目的

KatanA 側でできていた Mermaid / Draw.io / export / score の能力を落とさない kdr 公開 API を固定する。KatanA 側の型名完全一致は必須にせず、薄い adapter で接続できる情報量を守る。

### 書き込み範囲

- `crates/katana-diagram-renderer/src/renderer.rs`
- `crates/katana-diagram-renderer/src/exporter.rs`
- `crates/katana-diagram-renderer/src/lib.rs`
- `tests/`

### タスク

- [x] 2.1 KatanA 側の `Renderer` 相当能力を kdr の `Renderer` / `RenderInput` / `RenderOutput` / `RenderError` で表現する
- [x] 2.2 KatanA 側の `Exporter` 相当能力を kdr の `ExporterTrait` / `ExportInput` / `ExportOutput` / `ExportError` で表現する
- [x] 2.3 KatanA 側の `diagram_backend` DTO と kdr 公開 API の対応表を design / test に反映する
- [x] 2.4 Mermaid / Draw.io / export の失敗経路を KatanA 既存挙動と同等に表現する
- [x] 2.5 KatanA consumer adapter で必要な情報が DTO にそろっていることを kdr 側の回帰テストで確認する
- [x] 2.6 trait 境界を実装経由で通す回帰テストを追加する

### Definition of Done

- [x] 公開 API が KatanA 側の既存能力を落とさない
- [x] kdr 独自都合で field や format を削った結果、KatanA 側で機能差分の再実装が必要になっていない
...
KatanA 既存 Mermaid runtime と fixture coverage を kdr へ移す。

### 書き込み範囲

- `crates/katana-diagram-renderer/src/mermaid/`
- `vendor/mermaid/`
...
- [x] 3.5 Mermaid backend の KatanA 由来検証観点を kdr の unit test / integration test / reference compare に移す

### Definition of Done

- [x] PR #1 の簡略 SVG 生成ではなく、KatanA 既存 runtime と同等の出力である
- [x] version 固定と更新 recipe 改善は v0.1.1 に送られている
- [x] Mermaid 既存 fixture と KatanA 由来 runtime 検証観点が kdr 側で検証されている
...
KatanA 既存 Draw.io runtime と resource resolver を kdr へ移す。

### 書き込み範囲

- `crates/katana-diagram-renderer/src/drawio/`
- `vendor/drawio/`
...
- [x] 4.2 Draw.io resource、stencil、image、manifest を kdr 管理へ移す
...
- [x] 4.5 Draw.io backend の KatanA 由来検証観点を kdr の unit test / integration test / reference compare に移す

### Definition of Done

- [x] KatanA 既存 Draw.io tests と同等の検証観点が kdr 側にある
- [x] resource 解決が kdr 内で完結している
...
KatanA 既存 export runtime を kdr へ移し、HTML だけで完了扱いにしない。

### 書き込み範囲

- `crates/katana-diagram-renderer/src/exporter/`
- `tests/`
...
- [x] 5.5 export regression test と native visual export test の検証観点を kdr の `Exporter` 入力で移す
...
KatanA 既存の公式 reference 生成と画像 score を kdr に移す。

### 書き込み範囲

- `crates/katana-diagram-renderer-cli/src/`
- `tests/reference/`
- `scripts/` または kdr CLI 内部 module
- `.github/workflows/`
...
- [x] `kdr mermaid compare --min-score <score>` が画像 score を出力する
- [x] `kdr drawio compare --min-score <score>` が画像 score を出力する
...
kdr CLI を library の薄い利用者として整え、移植後の品質ゲートを自動化する。
...
- [x] 7.1 `kdr mermaid render / reference-update / compare / bench` を接続する
- [x] 7.2 `kdr drawio render / reference-update / compare / bench` を接続する
- [x] 7.3 `kdr export html / pdf / png / jpeg` を接続する
...
## 8. KatanA adapter readiness を kdr 側で確認する
...
KatanA 側の実組み込みは KatanA の関心として分離し、kdr では薄い adapter で必要になる公開能力が欠けていないことを確認する。

### 書き込み範囲

- `openspec/`
- `crates/katana-diagram-renderer/tests/`
- kdr 側 integration fixture

### タスク

- [x] 8.1 KatanA 側 `diagram_backend` 入出力と kdr `Render*` / `Export*` 入出力の対応を確認する
- [x] 8.2 Mermaid / Draw.io / export / score の既存能力を kdr 側テストと reference compare に寄せる
- [x] 8.3 KatanA 側実組み込みは kdr release 後の KatanA 側 task として扱うことを記録する

### Definition of Done

- [x] kdr 公開 API から KatanA adapter に必要な情報が欠落していない
- [x] KatanA 側実組み込みを kdr v0.1.0 完了条件に含めていない
- [x] KatanA 側に残したくない検証観点が kdr 側のテストまたは reference compare に移っている
...
- `docs/`
- `openspec/`
- `crates/katana-diagram-renderer/src/renderer/`
- `crates/katana-diagram-renderer/src/markdown/`
- `crates/kdr-linter/`
...
- [/] 9.4 kdr は独立機能なので、KatanA 側 interface の型名完全一致ではなく、KatanA 側の既存能力が落ちないことを完了基準にする
- [/] 9.5 KatanA 側を削ぐため、KatanA 由来テストの検証観点を kdr 側へ寄せる
