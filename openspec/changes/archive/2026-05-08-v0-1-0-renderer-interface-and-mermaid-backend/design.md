...
v0.1.0 の目的は、KatanA 既存の rendering/export runtime と検証責務をKDRに忠実移植すること。新規に簡略実装を作る作業ではない。

責務再整理後、KDRの最終責務は外部描画である。v0.1.0で移植したHTML/PDF/PNG/JPEG exportは、KDVに同等機能が入るまで維持し、KDV実装後に移譲・削除する。

PR #1 は close 済み。実装差分は土台にしない。レビュー履歴は、避けるべき方針と検証漏れの参照資料として使う。

## Goals

- KatanA の Mermaid 実装を kdr へ移植する
- KatanA の Draw.io 実装と resource 一式を kdr へ移植する
- KatanA の HTML / PDF / PNG / JPEG export 実装を kdr へ移植する
- KatanA の Mermaid / Draw.io reference 生成と ImageMagick 採点評価を kdr へ移植する
- KatanA 側でできていた Mermaid / Draw.io / export / score の能力を落とさない kdr 公開 API を提供する
- kdr は KatanA UI state に依存しない library と CLI を提供する

## Non-Goals

- PR #1 の簡略 Mermaid 実装を育てない
- KatanA 側でできていた機能を kdr 独自都合で縮小、改名、簡略化しない
- HTML のみ export を v0.1.0 完了扱いにしない
- SVG 文字列比較だけの score を採点評価として採用しない
- export CSS debug は要件変更により v0.1.x の release 番号から外し、旧 export/debug 論点はKDV移譲記録へ送る
- Mermaid.js / Draw.io.js の取り込み version 固定、最新版確認、取り込み just recipe は v0.1.1 に送る
- Mermaid ZenUML / unsupported fixture handling は v0.1.2 に送る
- Draw.io / Mermaid の score 改善は v0.1.4 の最終フェーズに送る
- CSV / PDF / Word / Excel / PPTX viewer rendering はKDVへ移譲し、KDRでは扱わない
...
kdr は KatanA を知らない library として設計する。

- `Renderer` / `Exporter` は kdr の独立 library として自然な公開 API にする
...
- vendor bundle、checksum、version pinning、resource manifest の整理は v0.1.1 で kdr 管理に固定する
...
2. KatanA 側で定義済みの interface と DTO を棚卸しし、kdr 公開 API で既存能力が落ちないことを照合する
...
5. HTML / PDF / PNG / JPEG export を移植し、既存回帰テストの検証観点を kdr 側で維持する
6. CLI、just、CI に reference-update / compare / bench / export を接続する
7. KatanA 側の consumer integration は KatanA 側の関心として分離し、kdr では adapter が必要とする能力を公開 API と検証で固定する

## KatanA Compatibility Boundary

kdr は KatanA のためだけの crate ではないため、KatanA 側の `diagram_backend` 型名をそのまま公開 API にすること自体は完了条件にしない。
...
- Mermaid / Draw.io の reference 生成、画像化、score compare を kdr 側で実行できる

KatanA 側の `DiagramBackendInput` / `DiagramBackendOutput` / `DiagramBackendError` / `DiagramThemeSnapshot` は、互換性確認の照合元として扱う。kdr の `RenderInput` / `RenderOutput` / `RenderError` / `RenderConfig` / `RenderPolicy` / `RenderContext` が同じ情報を保持できる場合、型名の完全一致は不要とする。

## KatanA Test Migration Boundary

KatanA 側のテストをそのまま残すのではなく、検証観点を kdr 側へ移す。

- Mermaid / Draw.io の runtime 解決、未導入、失敗経路、代表 diagram render は kdr の integration test で確認する
- Draw.io official fixture、Mermaid fixture、score floor は kdr の reference compare と full compare に寄せる
- export regression と native visual export は、KatanA の Markdown renderer ではなく kdr の `Exporter` 入力で確認する
- KatanA 固有の Markdown parser、preview state、workspace state、UI state の検証は kdr に持ち込まない

## Reference Artifact Ownership

kdr でも KatanA 本家と同じく、公式 reference の SVG / PNG は git 管理する。CI/CD は外部から公式描画結果を再取得・再生成しない。

- `reference-update` は開発者が明示的に実行する更新操作とする
- `compare` は git 管理済み reference と、その場で生成した kdr 出力を比較する
- CI/CD は `compare` のみを実行し、reference artifact を変更しない
- Mermaid.js / Draw.io.js の version 更新時だけ、v0.1.1 の update recipe で reference SVG / PNG を再生成する
...
Draw.io representative で現時点の移植実装が 99 点に届かない case は、`score-baseline.json` に既知下限として明示する。これは合格基準の隠蔽ではなく、現在値からの悪化を CI/CD で検知するための下限である。v0.1.0 release 時点では KatanA 側へまだ取り込まないため既存品質は劣化しない。KatanA 側へ取り込む前に、score 改善で下限を上げる作業は v0.1.4 の最終フェーズに送る。
...
- `v0.1.1`: Mermaid.js / Draw.io.js の version 固定、最新版確認、取り込み just recipe を整備する
- `v0.1.2`: Mermaid ZenUML / unsupported fixture handling を整備する
- `v0.1.3`: RenderInput theme application を整備する
- `v0.1.4`: Draw.io / Mermaid の score 改善。Jules 側で停滞した大きい作業を score 改善フェーズへ送る
- `v0.1.5`: KDV export handoff を記録する
- `v0.2.0`: CSV viewer renderingはKDVへ移譲する
- `v0.3.0`: PDF viewer renderingはKDVへ移譲する
- `v0.4.0`: Office viewer renderingはKDVへ移譲する
- `v0.4.x`: バグ取りと継続的な score 向上
- `v0.5.0`: CLI 公開

viewer rendering はKDVの責務である。KDRは外部描画、runtime asset、reference、scoreを扱い、既存exportはKDV移譲までの互換維持として扱う。

### v0.1.x Follow-up

export CSS debug は要件変更により v0.1.x の release 番号から外す。旧 export/debug 論点はKDV移譲記録で扱い、KDR側では新規 debug command を増やさない。

`v0.1.2` は、Mermaid ZenUML / unsupported fixture handling を固定し、v0.1.4 の score 改善が unsupported fixture で止まらない状態にする。

`v0.1.4` の score 改善（score improvement）は、full compare の未達を閉じる段階とする。KatanA 側への取り込み（consume）は、この score 改善後に判断する。
