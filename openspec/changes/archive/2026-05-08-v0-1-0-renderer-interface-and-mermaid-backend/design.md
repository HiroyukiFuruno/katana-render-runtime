...
v0.1.0 の目的は、KatanA 既存の rendering/export runtime と検証責務をKCFに忠実移植すること。新規に簡略実装を作る作業ではない。

責務再整理後、KCFの最終責務は外部描画である。v0.1.0で移植したHTML/PDF/PNG/JPEG exportは、KDVに同等機能が入るまで維持し、KDV実装後に移譲・削除する。
...
- CSV / PDF / Word / Excel / PPTX viewer rendering はKDVへ移譲し、KCFでは扱わない
...
viewer rendering はKDVの責務である。KCFは外部描画、runtime asset、reference、scoreを扱い、既存exportはKDV移譲までの互換維持として扱う。

### v0.1.x Follow-up

export CSS debug は要件変更により v0.1.x の release 番号から外す。旧 export/debug 論点はKDV移譲記録で扱い、KCF側では新規 debug command を増やさない。
