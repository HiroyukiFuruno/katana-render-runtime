...
Mermaid.js と Draw.io.js の取り込み version が曖昧なままだと、reference score、cache fingerprint、CI 再現性、KatanA consumer integration が揺れる。kdr が runtime asset の version 固定、最新版確認、取り込み、checksum、reference snapshot 更新を所有する。

## What Changes

- Mermaid.js の取り込み version を kdr 側で固定する
- Draw.io.js の取り込み version を kdr 側で固定する
...
- reference snapshot 更新は git 管理済み full / representative artifact を更新し、CI/CD の通常 compare では再生成しない
...
- `crates/katana-diagram-renderer/src/mermaid/` — runtime metadata / checksum integration
- `crates/katana-diagram-renderer/src/drawio/` — runtime metadata / checksum integration
