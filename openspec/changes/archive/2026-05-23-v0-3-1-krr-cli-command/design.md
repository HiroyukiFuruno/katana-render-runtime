...
旧 `kdr` は v0.3.0 までの挙動として扱う。v0.3.1 では `kdr` alias binary を同梱しない。
...
既存 `katana-diagram-renderer-cli` は crates.io 上の過去 package として残る。v0.3.1 では旧 package を更新せず、正本 package と release docs は `katana-render-runtime-cli` に置く。
...
旧 recipe 名は残さず、`krr-*` recipe を正本にする。
...
- 互換対象: `katana-diagram-renderer` wrapper、必要な `KDR_PLANTUML_*` fallback
...
- [Risk] `kdr` alias を残すと「最新も kdr で動く」と誤認される → v0.3.1 では alias binary を同梱しない。
...
- `katana-diagram-renderer-cli v0.3.1` は publish しない。旧 package は v0.3.0 の過去版として残す。
- `kdr` alias binary は v0.3.1 に同梱しない。
