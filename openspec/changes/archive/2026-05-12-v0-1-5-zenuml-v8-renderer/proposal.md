...
`@zenuml/core` が公開する `renderToSvg()` 関数を kcf の `DiagramV8Runtime` で
...
現状 kcf は `~~~mermaid` + `zenuml` キーワードのみを ZenUML として認識する。
...
- `crates/katana-canvas-forge/src/markdown/mermaid_renderer/zenuml_v8_runtime.rs` — 新レンダラー
- `crates/katana-canvas-forge/src/markdown/mermaid_renderer/js_runtime/render_zenuml_v8.js` — V8 用 JS ブリッジ
- `crates/katana-canvas-forge/vendor/zenuml-core/<version>/zenuml.js` — `@zenuml/core` vendor asset
