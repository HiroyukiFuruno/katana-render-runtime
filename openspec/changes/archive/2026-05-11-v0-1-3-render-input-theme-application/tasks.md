# Tasks: katana-diagram-renderer v0.1.3 RenderInput theme application
...
- [/] Issue #4: KatanA から light を `RenderInput` に渡しても dark で描画される。kdr 側で `RenderInput` の theme 情報を実描画と cache fingerprint に反映する。
...
- [x] 1.1 `RenderThemeMode` (Light / Dark) と `RenderThemeSnapshot` 型を `crates/katana-diagram-renderer/src/renderer/api.rs` に追加する
...
- [x] 3.2 `crates/katana-diagram-renderer/src/markdown/mermaid_renderer/render.rs` から `DiagramColorPreset::current()` 直接参照を撤去し、引数の preset を使う
...
- [x] 7.6 KatanA adapter は本 change の非目標とし、kdr 側では外部指定を受け取る DTO / request 経路だけを実装する
