...
一方で、移植時の full compare では Draw.io official diagrams の `i18n` と `link` が 99 点に届かず、代表ケースにも既知下限が残っている。これは v0.1.0 の公開を止めるのではなく、v0.1.x の最後に置く v0.1.4 の小規模 patch として扱う。

この範囲は旧 v0.1.1 として Jules 側へ渡していたが進捗が停滞しているため、実行時資産のバージョン固定（runtime asset version pinning）、export CSS 回帰修正、ZenUML 対応方針（ZenUML handling）を先に進めた後の最終段階に回す。
...
- unsupported fixture は暗黙 skip せず、v0.1.4 で扱わない理由と後続 change を明示する
...
- `openspec/changes/v0-1-4-score-improvement/` — 本 change の仕様とタスク
