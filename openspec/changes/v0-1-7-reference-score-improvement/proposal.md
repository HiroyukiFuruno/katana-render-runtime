...
この範囲は旧 v0.1.1 / v0.1.2 として Jules 側へ渡していたが、粒度が大きく進捗が停滞している。Mermaid ZenUML / unsupported fixture handling は v0.1.2 で閉じ、score 改善（score improvement）を v0.1.7 の最終フェーズへ送る。不要になった export CSS debug 計画はKCF release番号から外し、旧 export/debug 論点は v0.1.7 のKDV移譲記録で扱う。
...
- 不要になった export CSS debug 計画はKCF release番号から外し、旧 export/debug 論点は v0.1.7 のKDV移譲記録で扱う
