...
- `v0.1.2`: export CSS 回帰修正と macOS debug open を整備する
- `v0.1.3`: Mermaid ZenUML / unsupported fixture handling を整備する
- `v0.1.4`: Draw.io / Mermaid の score 改善。Jules 側で停滞した大きい作業を v0.1.x の最終フェーズへ送る
...
`v0.1.2` の export debug は、HTML / PDF / PNG / JPG を `/tmp` に出力し、macOS の既定アプリで開いて目視確認するための開発用コマンドとする。`floem` / `egui` などの画面表示ライブラリは core library に追加しない。core library は引き続き KatanA UI state、preview state、workspace state を持たない。自動採点の正本は ImageMagick score とし、export debug は実表示の確認補助に限定する。

`v0.1.3` は、Mermaid ZenUML / unsupported fixture handling を固定し、v0.1.4 の score 改善が unsupported fixture で止まらない状態にする。

`v0.1.4` の score 改善（score improvement）は、full compare の未達を v0.1.x の最後に閉じる段階とする。KatanA 側への取り込み（consume）は、この score 改善後に判断する。
