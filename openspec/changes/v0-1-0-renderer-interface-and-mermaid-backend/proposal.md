...
KatanA から Mermaid / Draw.io / export / 採点評価の責務をKCFへ移管する。KCF v0.1.0 は新規の簡略実装ではなく、KatanA 既存機能の忠実移植（transfer）を目的とする。

責務再整理後、KCFの最終責務は外部描画である。v0.1.0で移管したHTML/PDF/PNG/JPEG exportは、KDVに同等機能が入るまで維持し、KDV実装後に移譲・削除する。
...
- Draw.io / Mermaid の score 改善は v0.1.2 で扱う
- Mermaid.js / Draw.io.js の取り込み version 固定、最新版確認、取り込み just recipe は v0.1.1 で扱う
- export CSS 回帰修正と macOS debug open は v0.1.3 で扱う
- Mermaid ZenUML / unsupported fixture handling は v0.1.4 で扱う
- CSV / PDF / Word / Excel / PPTX viewer rendering は v0.1.0 transfer には含めない
- 新規のMarkdown viewer/export責務をKCFへ追加しない。KDV実装後は既存exportをKDVへ移譲する
