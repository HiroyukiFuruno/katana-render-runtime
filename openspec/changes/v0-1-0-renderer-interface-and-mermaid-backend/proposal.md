...
KatanA v0.22.11 で描画責務を kcf へ移管する。kcf はまず Mermaid rendering の interface と backend を確立し、KatanA が git dependency として利用できる状態にする。これにより KatanA の検証範囲が描画実装を含まなくなり、リリースごとの負荷が下がる。
...
- `v0.1.0` として release tag を切り、KatanA が取り込めるようにする。
