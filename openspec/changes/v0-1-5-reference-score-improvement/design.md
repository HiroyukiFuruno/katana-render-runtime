...
unsupported fixture は暗黙 skip しない。Mermaid ZenUML / unsupported fixture handling は v0.1.4 で固定し、v0.1.5 はその metadata を前提に supported pattern の score 改善へ集中する。
...
修正しない既知差分は、理由と現在 score を report に残す。ただし、その case が supported fixture である限り、99 未満のまま v0.1.5 完了にはしない。どうしても v0.1.5 で直せない場合は、ユーザー確認を受けて別 change へ明示的に送る。
...
Mermaid ZenUML 由来の failure は v0.1.4 の対象として扱い、v0.1.5 の score 改善完了条件には含めない。
