...
- **WHEN** `krr plantuml render --input <file> --output <file>` を実行する
...
システムは、`krr plantuml render`、`krr plantuml reference-update`、`krr plantuml compare`、`krr plantuml bench` を提供しなければならない（MUST）。CLI（command line interface: コマンドで使う入口）は Mermaid / Draw.io と同じ action 構造を使わなければならない（MUST）。

#### Scenario: render command を parse する

- **WHEN** `krr plantuml render --input in.puml --output out.svg` を parse する
...
- **WHEN** `krr plantuml render --input in.puml` を実行する
...
- **WHEN** `krr plantuml compare --fixtures tests/fixtures/plantuml/official --min-score 100` を実行する
...
システムは、KDV が HTML export に埋め込める SVG または raw code block を返さなければならない（MUST）。KRR は `<figure data-kdv-diagram="plantuml">` の生成責務を持ってはならない（MUST NOT）。

#### Scenario: KDV が PlantUML SVG を埋め込む

- **GIVEN** KDV が `plantuml` fenced code を KRR に渡す
- **WHEN** KRR が `RenderOutput.svg` を返す
- **THEN** KDV はその SVG を `<figure data-kdv-diagram="plantuml">` 内へ埋め込める
- **THEN** KRR は KDV 固有 HTML wrapper を返さない

#### Scenario: KDV が PlantUML raw code fallback を埋め込む

- **GIVEN** KRR が warning 付き raw code block を返す
...
- **THEN** 描画 source には KRR 独自の色上書きではなく PlantUML の `!theme cyborg` 相当が渡される
- **THEN** `cache_fingerprint` は theme 未指定時と異なる

#### Scenario: CLI help と API で theme 候補を確認できる

- **GIVEN** 利用者が `krr plantuml render --help` を表示する
